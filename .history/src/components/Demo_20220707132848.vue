  ...
  if(!(sessionStorage.getItem("dataA")&&sessionStorage.getItem("dataB"))){
    ...
    .then(axios.spread(function (dataA, dataB) {
      ...
      Info.list.map((item,index)=>{
       if(item&&index<10){//不为null,
         
          let data_now=(new Date(item.create_time.split(" ")[0])).getTime()
          console.log(data_now,item.create_time)

           let temp=allMes[data_now];
          if(!temp){
            Info.sortList[data_now]=[].concat(item.create_time)
          }else{
            Info.sortList[data_now]=Info.sortList[data_now].concat(item.create_time)
          }
       }
